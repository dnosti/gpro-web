...
namespace gpro_web.Services
{
  ...
  public class UsuarioService : IUsuarioService
  {
    ...
    public List<Object> GetAll()
    {
      ...
      var _usuarios = from u in _context.Usuario select u;

      if (_usuarios.ToList().Count() == 0)
      {
        return null;
      }
      else
      {
        for (int i = 0; i < _usuarios.ToList().Count; i++)
        {
          var aux = from usr in _context.Usuario
                    where usr.IdEmpleadoNavigation.IdEmpleado.Equals(_usuarios.ToList().ElementAt(i).IdEmpleado)
                    select usr;
